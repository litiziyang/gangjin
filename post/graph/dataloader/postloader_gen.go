...
type PostLoaderConfig struct {
	...
	Fetch func(keys []uint64) ([]*model.Post, []error)
...
func (l *PostLoader) Load(key uint64) (*model.Post, error) {
...
func (l *PostLoader) LoadThunk(key uint64) func() (*model.Post, error) {
	l.mu.Lock()
...
func (l *PostLoader) LoadAll(keys []uint64) ([]*model.Post, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *PostLoader) LoadAllThunk(keys []uint64) func() ([]*model.Post, []error) {
	results := make([]func() (*model.Post, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *PostLoader) Prime(key uint64, value *model.Post) bool {
	l.mu.Lock()
	var found bool
...
func (l *PostLoader) Clear(key uint64) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *PostLoader) unsafeSet(key uint64, value *model.Post) {
	if l.cache == nil {
		l.cache = map[uint64]*model.Post{}
	}
	l.cache[key] = value
}
...
func (b *postLoaderBatch) keyIndex(l *PostLoader, key uint64) int {
